...
export const Input: React.FC<IInputPropos> = (propos) => {
	...
	// @audit i need this function too
	...
	const transferErc1155SingleToPortal = async (
		...
		try {
			if (rollups && provider) {
				...
				const erc1155SinglePortalAddress =
					rollups.erc1155SinglePortalContract.address;

				const tokenContract = signer
					? IERC1155__factory.connect(contractAddress, signer)
					: IERC1155__factory.connect(contractAddress, provider);
				...
				// Transfer
				rollups.erc1155SinglePortalContract.depositSingleERC1155Token(
					contractAddress,
					propos.dappAddress,
					id,
					amount,
	...
	const transferErc1155BatchToPortal = async (
		...
		try {
			if (rollups && provider) {
				...
				const erc1155BatchPortalAddress =
					rollups.erc1155BatchPortalContract.address;
				...
				if (!currentApproval) {
					...
					if (!event) {
						throw Error(
							`could set approval for DAppERC1155Portal(${erc1155BatchPortalAddress})  (signer: ${signerAddress}, tx: ${tx.hash})`
						);
					}
				}
